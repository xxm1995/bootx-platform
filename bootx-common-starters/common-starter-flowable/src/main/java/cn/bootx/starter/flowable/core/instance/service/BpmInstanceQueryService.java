...
public class BpmInstanceQueryService {
    ...
    public PageResult<InstanceInfo> pageMyApply(PageParam pageParam) {
        val instanceQuery = historyService.createHistoricProcessInstanceQuery()
                .startedBy(String.valueOf(SecurityUtil.getUserId())).orderByProcessInstanceStartTime().desc();
    ...
    public List<String> getCurrentNodes(String instanceId) {
        ...
        return runtimeService.createExecutionQuery().processInstanceId(instanceId).list().stream()
                .map(Execution::getActivityId).distinct().filter(Objects::nonNull).collect(Collectors.toList());
    }
    ...
    public List<LabelValue> getBackNodes(String instanceId) {
        ...
        List<HistoricActivityInstance> historicActivityList = historyService.createHistoricActivityInstanceQuery()
                .processInstanceId(instanceId).list();
        List<String> historicActivities = historicActivityList.stream().map(HistoricActivityInstance::getActivityId)
                .collect(Collectors.toList());
        // 当前执行的节点
        List<String> currentNodes = this.getCurrentNodes(instanceId);

        return process.getFlowElements().stream().filter(flowElement -> flowElement instanceof Task)
                .filter(flowElement -> historicActivities.contains(flowElement.getId()))
                .filter(flowElement -> !currentNodes.contains(flowElement.getId())).map(o -> (Task) o)
    ...
    public List<InstanceInfo> convertInstanceInfo(List<String> instanceIds) {
        Map<String, BpmInstance> bpmInstanceMap = bpmInstanceManager.findAllByInstanceIds(instanceIds).stream()
        ...
        return instanceIds.stream().map(instanceId -> {
            ...
            return new InstanceInfo().setName(bpmInstance.getInstanceName()).setInstanceId(bpmInstance.getInstanceId())
                    .setState(bpmInstance.getState()).setStartTime(bpmInstance.getStartTime())
                    .setEndTime(bpmInstance.getEndTime()).setStartUserId(bpmInstance.getStartUserId())
                    .setStartUserName(bpmInstance.getStartUserName()).setDefMame(bpmInstance.getDefName());
        }).collect(Collectors.toList());
    }
