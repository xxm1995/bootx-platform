...
@Component
@ReadingConverter
public class DataVersionRecordHandler implements InnerInterceptor {
    ...
    public void beforePrepare(StatementHandler sh, Connection connection, Integer transactionTimeout) {
        ...
        if (sct == SqlCommandType.INSERT || sct == SqlCommandType.UPDATE || sct == SqlCommandType.DELETE) {
            ...
            try {
                ...
                // 判断是否需要进行处理
                if (!this.allowProcess(statement)){
                    return;
                }
     ...
     * 判断哪些SQL需要处理
     */
    protected boolean allowProcess(Statement statement) {
        String tableName;
        if (statement instanceof Insert) {
        ...
        return Objects.nonNull(tableInfo.getEntityType().getAnnotation(DataVersionLog.class));
    }
