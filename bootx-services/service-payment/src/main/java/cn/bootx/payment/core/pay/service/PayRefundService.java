...
public class PayRefundService {
    ...
    private void refundPayment(Payment payment, List<RefundModeParam> refundModeParams) {
        ...
        List<PayModeParam> payModeParams = refundModeParams.stream().map(RefundModeParam::toPayModeParam)
                .collect(Collectors.toList());
        ...
        this.doHandler(payment, paymentStrategyList, (strategyList, paymentObj) -> {
            ...
            try {
                ...
                // 记录退款失败的记录
                BigDecimal amount = refundModeParams.stream().map(RefundModeParam::getAmount).reduce(BigDecimal.ZERO,
                        BigDecimal::add);
    ...
    public void saveRefund(Payment payment, BigDecimal amount, List<RefundModeParam> refundModeParams) {
        List<RefundableInfo> refundableInfos = refundModeParams.stream().map(RefundModeParam::toRefundableInfo)
                .collect(Collectors.toList());
        ...
        RefundRecord refundRecord = new RefundRecord().setRefundRequestNo(AsyncRefundLocal.get())
                .setRefundableInfo(refundableInfos).setAmount(amount)
                .setRefundableBalance(payment.getRefundableBalance()).setClientIp(ip).setPaymentId(payment.getId())
                .setBusinessId(payment.getBusinessId()).setUserId(SecurityUtil.getUserIdOrDefaultId())
                .setRefundTime(LocalDateTime.now()).setTitle(payment.getTitle())
                .setErrorMsg(AsyncRefundLocal.getErrorMsg()).setErrorCode(AsyncRefundLocal.getErrorCode())
