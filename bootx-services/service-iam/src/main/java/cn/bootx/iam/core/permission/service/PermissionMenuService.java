...
public class PermissionMenuService {
    ...
    public PermissionMenuDto add(PermissionMenuParam param) {
        ...
        if(Objects.nonNull(pid)) {
            //设置父节点不为叶子节点
            permissionMenuManager.setLeaf(pid, false);
        }
        param.setLeaf(true);
    ...
    public PermissionMenuDto update(PermissionMenuParam param){
        ...
        if(!permissionMenuManager.existsByParentId(permission.getId())) {
            permission.setLeaf(true);
        }
        PermissionMenuDto permissionDto = permissionMenuManager.updateById(permission).toDto();
        ...
        if (!Objects.equals(oldPid,pid)){
            ...
            if(Objects.nonNull(oldPid) && !permissionMenuManager.existsByParentId(oldPid)) {
                permissionMenuManager.setLeaf(oldPid, true);
            }
