...
public class DbAppender extends AppenderBase<ILoggingEvent> {
    ...
    /**
     * 注意, 在里面调用异步方法的时候不可以触发打印日志, 否则会死循环 同步方法中触发了日志打印, 不会触发该方法 定时写入到数据库中 使用Mybatis
     * Plus进行分表, 分日/周/月进行保存 设置一个队列, 然后定时从队列中取出日志进行保存
     */
    @Override
    protected void append(ILoggingEvent eventObject) {
        ...
        LogInfo logInfo = new LogInfo().setMessage(eventObject.getFormattedMessage())
                .setClassName(eventObject.getThreadName()).setLevel(eventObject.getLevel().levelStr)
                .setThreadName(eventObject.getThreadName()).setTime(LocalDateTime.ofInstant(instant, zone))
